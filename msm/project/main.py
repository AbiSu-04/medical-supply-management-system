...
def search():
    ...
    if request.method == 'POST':
        ...
        post = Addmpnpd.query.filter_by(medicine=name).first()
        pro = Addmpnpd.query.filter_by(product=name).first()
...
@app.route("/addmpnpd", methods=['GET', 'POST'])
def addmpnpd():
    if request.method == 'POST':
       
        new_medicine = request.form.get('medicine')
        new_product = request.form.get('product')
        
       
        if new_medicine and new_product:
           
            push = Addmpnpd(medicine=new_medicine, product=new_product)
            ...
            flash("Please provide both medicine and product", "danger")
